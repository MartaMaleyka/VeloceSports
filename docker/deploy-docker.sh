...
rm -rf "$DEPLOY_HOME/mariadb" "$DEPLOY_HOME/mariadb-data" "$DEPLOY_HOME/mariadb-local" \
       "$DEPLOY_HOME/mariadb-10.11.13-linux-systemd-x86_64" "$DEPLOY_HOME/mariadb.tar.gz" 2>/dev/null || true

echo "==> Limpiando node_modules nativos (Docker reconstruye)..."
rm -rf "$APP_DIR/node_modules" \
       "$APP_DIR/apps/web/node_modules" \
       "$APP_DIR/apps/backend/node_modules" 2>/dev/null || true
find "$APP_DIR/packages" -maxdepth 2 -name node_modules -type d -exec rm -rf {} + 2>/dev/null || true
