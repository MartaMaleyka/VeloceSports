export const en = {
  common: {
    appName: 'SquadVeloce',
    brandSlogan: 'Where talent is born',
    home: 'Home',
    logout: 'Sign out',
    required: 'required',
    closeNotification: 'Close notification',
    close: 'Close',
    cancel: 'Cancel',
    save: 'Save',
    saving: 'Saving…',
    create: 'Create',
    edit: 'Edit',
    view: 'View',
    search: 'Search',
    actions: 'Actions',
    moreActions: 'More actions',
    loading: 'Loading…',
    retry: 'Retry',
    copy: 'Copy',
    copied: 'Copied',
    active: 'Active',
    inactive: 'Inactive',
    suspended: 'Suspended',
    confirm: 'Confirm',
    back: 'Back',
  },
  players: {
    photo: {
      title: "Photo of {name}",
      subtitle: 'Upload a photo of {name}. Only you and the academy team will see it.',
      choose: 'Choose photo',
      hint: 'JPG, PNG or WebP · max 5 MB',
      upload: 'Upload',
      delete: 'Remove photo',
      deleteConfirm: 'Are you sure you want to remove your child’s photo?',
      deleteConfirmSelf: 'Are you sure you want to remove your photo?',
      deleteYes: 'Yes, remove',
      change: 'Change photo',
      success: 'Photo updated',
      deleted: 'Photo removed',
      errors: {
        tooLarge: 'The photo cannot exceed 5 MB',
        format: 'Unsupported format. Use JPG, PNG or WebP',
        crop: 'Adjust the square crop before uploading',
        generic: 'We could not upload the photo. Check your connection and try again.',
        delete: 'We could not remove the photo. Try again.',
      },
    },
  },
  profile: {
    menuLabel: 'My profile',
    logoutLabel: 'Sign out',
    title: 'My profile',
    avatarLabel: 'Avatar for {name}',
    fields: {
      firstName: 'First name',
      lastName: 'Last name',
      email: 'Email',
      roles: 'Roles',
      academy: 'Academy',
    },
    emailChangeHint: 'Your next sign-in will use the new email address.',
    saveProfile: 'Save changes',
    profileSaved: 'Profile updated',
    passwordSection: 'Change password',
    currentPassword: 'Current password',
    newPassword: 'New password',
    confirmPassword: 'Confirm new password',
    revokeOtherSessions: 'Sign out on other devices',
    revokeOtherSessionsHint: 'You will stay signed in on this device.',
    changePassword: 'Update password',
    passwordChanged: 'Password updated',
    passwordMismatch: 'New passwords do not match',
    passwordWeak: 'At least 8 characters with one letter and one number',
    errors: {
      title: 'We could not complete the action',
      generic: 'Something went wrong. Please try again.',
      emailTaken: 'That email is already in use',
      wrongPassword: 'Current password is incorrect',
    },
  },
  auth: {
    login: {
      tagline: 'Youth football academies',
      prompt: 'Sign in to access your dashboard',
      formTitle: 'Welcome back',
      formSubtitle: 'Enter your credentials to continue',
      footerNote: 'One login for admins, coaches, and parents.',
      heroFeatures: {
        capture: 'Live capture from the pitch',
        progress: 'Player tracking and match reports',
        alerts: 'Alerts when your child shines in a match',
      },
      emailLabel: 'Email address',
      passwordLabel: 'Password',
      submit: 'Sign in',
      errorTitle: 'Sign-in error',
      successToast: 'You signed in successfully',
      sessionEndedInactivity:
        'Your session ended due to inactivity. Please sign in again.',
      errors: {
        emailRequired: 'Enter your email address',
        emailInvalid: 'Invalid email address',
        passwordRequired: 'Enter your password',
        passwordMin: 'Password must be at least 8 characters',
        invalidCredentials: 'We could not sign you in. Check your credentials.',
        sessionFailed: 'We could not save your session. Try again.',
        network: 'We could not reach the server. Check your connection and try again.',
      },
      noAcademy: "Don't have an account?",
      goToSignup: 'Sign up for free',
    },
    signup: {
      formTitle: 'Create your account',
      formSubtitle: 'Sign up as an independent parent or register your academy.',
      heroTagline: 'Join SquadVeloce and capture every match',
      heroFeatures: {
        noAcademy: 'Use it solo or register your whole academy',
        capture: 'Capture live actions from the pitch',
        free: 'Free for a single player',
      },
      modeLabel: 'Account type',
      modePersonal: 'Just me and my child',
      modeAcademy: 'I have an academy',
      academyName: 'Academy name',
      adminFirstName: 'Your first name',
      adminLastName: 'Your last name',
      academyPendingHint:
        'A super admin will review and approve your academy before you can sign in.',
      childSectionTitle: "Your child's info",
      parentFirstName: 'Your first name',
      parentLastName: 'Your last name',
      childFirstName: "Player's first name",
      childLastName: "Player's last name",
      childJerseyNumber: 'Jersey number (optional)',
      submit: 'Create account',
      pendingTitle: 'Your request was submitted',
      pendingBody:
        'An administrator will review your account ({email}). You can sign in once it is approved.',
      haveAccount: 'Already have an account?',
      goToLogin: 'Sign in',
      errors: {
        required: 'This field is required',
        generic: 'We could not create your account. Try again.',
      },
    },
    resetPassword: {
      title: 'Reset password for {name}',
      description:
        'A temporary password will be created for {name}. They must change it on first login. All active sessions will be closed.',
      modeLabel: 'How do you want to set the password?',
      modeGenerate: 'Generate automatic password',
      modeGenerateHint: 'Recommended. The system creates a secure password.',
      modeManual: 'Write a password',
      modeManualHint: 'You define the temporary password and share it with the user.',
      newPassword: 'Temporary password',
      confirmPassword: 'Confirm password',
      submit: 'Reset password',
      successTitle: 'Password reset',
      successHint: 'The password was updated successfully.',
      tempPasswordLabel: 'Temporary password',
      copyPassword: 'Copy password',
      oneTimeWarning:
        'Share it with the user through a secure channel (WhatsApp, message). You will not see it again.',
      manualApplied: 'The password you typed was applied. Share it with the user.',
      mustChangeReminder: 'The user must change it when they sign in.',
      strengthOk: 'Valid password',
      errors: {
        weak: 'At least 8 characters, including a letter and a number',
        mismatch: 'Passwords do not match',
        generic: 'We could not reset the password. Try again.',
      },
    },
    forcedChange: {
      title: 'You must change your password to continue',
      subtitle:
        'Your administrator set a temporary password. Choose a new one to protect your account.',
      newPassword: 'New password',
      confirmPassword: 'Confirm password',
      submit: 'Save new password',
      logout: 'Sign out',
      success: 'Password updated. Welcome back.',
      strengthOk: 'Valid password',
      errors: {
        weak: 'At least 8 characters, including a letter and a number',
        mismatch: 'Passwords do not match',
        generic: 'We could not update the password. Try again.',
      },
    },
  },
  roles: {
    super_admin: 'Super Administrator',
    academy_admin: 'Academy administrator',
    coach: 'Coach',
    parent: 'Parent / guardian',
    player: 'Player',
  },
  nav: {
    home: 'Home',
    main: 'Main navigation',
    plans: 'Plans',
    academies: 'Academies',
    personalAccounts: 'Personal accounts',
    users: 'Users',
    categories: 'Categories',
    players: 'Players',
    matches: 'Matches',
    actions: 'Action catalog',
    settings: 'My academy',
    children: 'My children',
    parentCalendar: 'Calendar',
    parentNotifications: 'Notifications',
    playerProfile: 'My profile',
    playerMatches: 'My matches',
    playerCalendar: 'My calendar',
    playerReports: 'My reports',
    superAdmins: 'Super admins',
    billing: 'Billing',
    audit: 'Audit log',
    reports: 'Reports',
    analysis: 'Analysis',
  },
  platform: {
    plans: {
      title: 'Plans',
      description: 'Manage platform plans and limits.',
      create: 'Create plan',
      edit: 'Edit plan',
      editPlan: 'Edit plan',
      popular: 'Recommended',
      empty: 'No plans yet.',
      emptyAction: 'Create first plan',
      searchPlaceholder: 'Search plans…',
      filterStatus: 'Status',
      allStatuses: 'All statuses',
      statusLabel: 'Status',
      limits: {
        section: 'Includes',
        players: '{count} players',
        categories: '{count} categories',
        users: '{count} users',
        matches: '{count} matches/month',
      },
      kpis: {
        total: 'Total plans',
        active: 'Active plans',
        inactive: 'Inactive plans',
      },
      pricePerMonth: '{price}/mo',
      pricePerYear: '{price}/yr',
      pricing: {
        annualFee: '{price} USD/year',
        pricePerPlayer: '{price}/active player/mo',
      },
      columns: {
        name: 'Name',
        price: 'Pricing',
        limits: 'Limits',
        status: 'Status',
        actions: 'Actions',
      },
      billing: {
        monthly: 'Monthly',
        yearly: 'Yearly',
      },
      form: {
        name: 'Plan name',
        description: 'Description',
        annualFee: 'Annual fee (USD)',
        annualFeeHint: 'Charged once per year in the academy anniversary month.',
        pricePerPlayer: 'Price per active player (USD/mo)',
        pricePerPlayerHint: 'Variable monthly fee based on players with active status.',
        maxPlayers: 'Max players',
        maxCategories: 'Max categories',
        maxUsers: 'Max users',
        maxMatches: 'Max matches/month',
        status: 'Status',
        submitCreate: 'Create plan',
        submitEdit: 'Save changes',
      },
      activate: 'Activate',
      deactivate: 'Deactivate',
      successCreate: 'Plan created successfully.',
      successUpdate: 'Plan updated successfully.',
      successStatus: 'Plan status updated.',
      confirmDeactivate: 'Deactivate this plan? Existing academies are not removed.',
    },
    academies: {
      title: 'Academies',
      description: 'Manage tenants, plans, and access.',
      create: 'New academy',
      edit: 'Edit academy',
      detail: 'Academy detail',
      empty: 'No academies registered.',
      emptyAction: 'Create first academy',
      searchPlaceholder: 'Search by name or slug…',
      filterStatus: 'Status',
      filterPlan: 'Plan',
      allStatuses: 'All statuses',
      allPlans: 'All plans',
      noPlan: 'No plan',
      cardUsers: '{count} users in the academy',
      kpis: {
        total: 'Total academies',
        active: 'Active academies',
        suspendedInactive: 'Suspended / inactive',
        platformUsers: 'Platform users',
      },
      columns: {
        name: 'Academy',
        plan: 'Plan',
        users: 'Users',
        status: 'Status',
        actions: 'Actions',
      },
      form: {
        name: 'Academy name',
        slug: 'Identifier (slug)',
        slugHint: 'Optional. Auto-generated if left empty.',
        plan: 'Plan',
        billingAnchorDay: 'Billing cut-off day',
        billingAnchorDayHint: 'The academy will be billed on this day of each month (1–31).',
        adminEmail: 'Initial admin email',
        adminEmailHint: 'An academy_admin account will be created with a temporary password.',
        submitCreate: 'Create academy',
        submitEdit: 'Save changes',
      },
      status: {
        activate: 'Activate',
        suspend: 'Suspend',
        deactivate: 'Deactivate',
        confirmSuspend: 'Suspend this academy? Its users will not be able to sign in.',
        confirmInactive: 'Mark academy as inactive?',
      },
      suspension: {
        billing: 'Suspended for billing',
        manual: 'Suspended',
      },
      reactivate: {
        action: 'Reactivate',
        title: 'Reactivate academy',
        confirmDescription:
          'Reactivate {name}? Its users will be able to sign in again.',
        overdueWarning:
          'This academy has {count} overdue invoice(s) still pending. Reactivate anyway?',
        success: 'Academy reactivated successfully.',
        afterPaymentHint:
          'Academy {name} is still suspended. Reactivate it now?',
        dismissHint: 'Dismiss',
      },
      tempPassword: {
        title: 'Administrator temporary password',
        description: 'Copy this password now. It will not be shown again.',
        adminLabel: 'Administrator',
        passwordLabel: 'Temporary password',
        dismiss: 'Got it',
      },
      users: {
        title: 'Academy users',
        create: 'Add user',
        empty: 'No users besides the administrator.',
        columns: {
          email: 'Email',
          role: 'Role',
          status: 'Status',
          actions: 'Actions',
        },
        form: {
          email: 'Email address',
          role: 'Role',
          submit: 'Create user',
        },
        searchPlaceholder: 'Search users…',
        limitExceeded: 'The academy reached its plan user limit.',
        limitReached: 'You reached the plan user limit. Upgrade the plan or deactivate a user.',
        addDisabled: 'User limit reached',
        kpis: {
          total: 'Total users',
          limit: 'Plan limit',
          limitUsage: '{current} of {max}',
          admins: 'Administrators',
          coaches: 'Coaches',
          parents: 'Parents / guardians',
        },
        successCreate: 'User created successfully.',
        successStatus: 'User status updated.',
      },
      successCreate: 'Academy and administrator created.',
      successUpdate: 'Academy updated successfully.',
      successStatus: 'Academy status updated.',
      billing: {
        anchorDay: 'Cut-off day',
        currentPeriod: 'Current period',
        nextPeriod: 'Next period',
        periodRange: '{start} — {end}',
        estimateLabel: 'Billing estimate',
        estimate: 'Annual fee: {annual} · Est. monthly: {monthly} ({players} players × {perPlayer})',
      },
    },
    accounts: {
      approve: 'Approve',
      approveConfirmTitle: 'Approve this account?',
      approveConfirmDescription: '{name} will be able to sign in immediately.',
      successApprove: 'Account approved',
      successReject: 'Account rejected',
      pendingBanner: '{count} account(s) awaiting your approval.',
      reasonLabel: 'Reason',
      reject: {
        action: 'Reject',
        title: 'Reject account',
        description: 'This action cannot be undone. {name} will not be able to sign in.',
        reasonLabel: 'Reason (optional, saved to the record)',
        reasonPlaceholder: 'E.g.: incomplete information or suspected spam',
        confirm: 'Reject account',
      },
    },
    personalAccounts: {
      title: 'Personal accounts',
      description: 'Parents without an academy who signed up on their own — they need your approval to sign in.',
      searchPlaceholder: 'Search account…',
      empty: 'No personal accounts yet.',
      columns: {
        name: 'Account',
        requestedAt: 'Requested',
      },
      kpis: {
        total: 'Total',
        pending: 'Pending',
        approved: 'Approved',
        rejected: 'Rejected',
      },
      approvalStatus: {
        pending: 'Pending',
        approved: 'Approved',
        rejected: 'Rejected',
      },
    },
    superAdmins: {
      title: 'Super administrators',
      description: 'Manage global platform access.',
      create: 'Add super admin',
      empty: 'No other super administrators.',
      searchPlaceholder: 'Search super admins…',
      kpis: {
        active: 'Active super admins',
        total: 'Total super admins',
      },
      columns: {
        email: 'Email',
        status: 'Status',
        lastLogin: 'Last sign-in',
        actions: 'Actions',
      },
      form: {
        email: 'Email address',
        submit: 'Create super admin',
      },
      lastLoginNever: 'Never',
      cannotDeactivateLast: 'At least one active super admin must remain.',
      successCreate: 'Super administrator created.',
      successStatus: 'Status updated.',
      tempPassword: {
        title: 'Temporary password',
        description: 'Deliver this password securely to the new super admin.',
      },
    },
    errors: {
      generic: 'We could not complete the action. Try again.',
      forbidden: 'You do not have permission for this action.',
      notFound: 'Resource not found.',
      network: 'We could not reach the server.',
    },
    billing: {
      title: 'Billing',
      description: 'Manage academy invoices, payments, and due dates.',
      create: 'Create invoice',
      processOverdue: 'Process overdue',
      processOverdueSuccess: 'Processed {count} overdue invoice(s).',
      empty: 'No invoices yet.',
      emptyAction: 'Create first invoice',
      searchPlaceholder: 'Search by academy…',
      filterStatus: 'Status',
      filterAcademy: 'Academy',
      filterMonth: 'Month',
      allStatuses: 'All statuses',
      allAcademies: 'All academies',
      downloadPdf: 'Download PDF',
      markPaid: 'Mark paid',
      markPending: 'Mark pending',
      cancel: 'Cancel invoice',
      confirmCancel: 'Cancel this invoice? This cannot be undone.',
      successCreate: 'Generated {count} invoice(s) for the period.',
      successPayment: 'Payment status updated.',
      successCancel: 'Invoice cancelled.',
      status: {
        pending: 'Pending',
        paid: 'Paid',
        overdue: 'Overdue',
        cancelled: 'Cancelled',
      },
      invoiceType: {
        monthly: 'Monthly',
        annual: 'Annual',
      },
      breakdown: '{count} players × {price}',
      academyStatus: {
        current: 'Up to date',
        pending: 'Pending',
        overdue: 'Overdue',
      },
      columns: {
        academy: 'Academy',
        plan: 'Plan',
        type: 'Type',
        period: 'Period',
        amount: 'Amount',
        dueDate: 'Due date',
        status: 'Status',
        actions: 'Actions',
      },
      kpis: {
        totalBilled: 'Billed this month',
        pending: 'Pending',
        overdue: 'Overdue',
        paid: 'Paid',
      },
      form: {
        academy: 'Academy',
        periodMonth: 'Billing period (month)',
        notes: 'Notes',
        submit: 'Generate period invoices',
        periodHint: 'Leave empty to use the current period based on the cut-off day.',
      },
      alerts: {
        upcomingTitle: 'Payment due soon',
        upcomingDescription: 'You have an invoice for {amount} due on {date}. Pay before the due date to avoid academy suspension.',
        overdueTitle: 'Overdue invoice',
        overdueDescription: 'You have an overdue invoice for {amount} (due {date}). Your academy may be suspended if payment is not resolved. Contact platform support.',
      },
      summary: {
        currentPlan: 'Current plan',
        billingStatus: 'Billing status',
        anchorDay: 'Cut-off day',
        anchorDayValue: 'Day {day} of each month',
        currentPeriod: 'Current period',
        nextPeriod: 'Next period',
        periodRange: '{start} — {end}',
        dueDate: 'Estimated due date',
      },
    },
    audit: {
      title: 'Audit log',
      description: 'Platform action history. Read-only.',
      searchPlaceholder: 'Search by actor, academy, action…',
      filterAcademy: 'Academy',
      filterActor: 'Actor',
      filterEntity: 'Entity',
      filterAction: 'Action',
      filterDateFrom: 'From',
      filterDateTo: 'To',
      allAcademies: 'All academies',
      allActors: 'All actors',
      allEntities: 'All entities',
      allActions: 'All actions',
      platformScope: 'Platform',
      unknownActor: 'Unknown user (#{id})',
      unknownTenant: 'Academy unavailable',
      empty: 'No audit events yet.',
      filteredEmpty: 'No events match the current filters.',
      viewDetail: 'View detail',
      detailTitle: 'Event detail',
      detailWhen: 'Date and time',
      detailActor: 'Actor',
      detailAcademy: 'Academy',
      detailEntity: 'Affected entity',
      detailAction: 'Action',
      noChanges: 'No change data recorded.',
      changesTitle: 'Changes',
      field: 'Field',
      before: 'Before',
      after: 'After',
      timezoneHint: 'Times shown in Panama time (UTC−5)',
      pagination: 'Page {page} of {totalPages} · {total} events',
      columns: {
        when: 'Date / time',
        actor: 'Actor',
        action: 'Action',
        entity: 'Entity',
        target: 'Target',
        actions: 'Actions',
      },
      kpis: {
        totalEvents: 'Events in range',
        topAction: 'Most frequent action',
        topActionCount: '{action} ({count})',
        noData: 'No data',
      },
      entities: {
        academy: 'Academy',
        user: 'User',
        plan: 'Plan',
        invoice: 'Invoice',
        super_admin: 'Super admin',
      },
      actions: {
        create: 'Create',
        update: 'Update',
        status_change: 'Status change',
        reactivate: 'Reactivate',
        cancel: 'Cancel',
        payment_status_change: 'Payment change',
        overdue_processed: 'Overdue processed',
        plan_limit_exceeded: 'Plan limit exceeded',
      },
    },
  },
  dashboard: {
    superAdmin: {
      greeting: 'Hello, {role}',
      greetingMulti: 'Unified dashboard ({roles})',
      description: 'Full SaaS business control: academies, plans, billing, and metrics.',
      welcome:
        'Platform health at a glance. Review growth, collections, and what needs attention.',
      home: {
        title: 'Home',
        description: 'Business health at a glance.',
        heroEyebrow: 'Platform',
        heroTitle: 'SquadVeloce Platform',
        heroSubtitle: 'Business health at a glance.',
        heroPillAcademies: '{count} active academies',
        heroPillUsers: '{count} total users',
        heroPillMrr: '{amount} MRR',
        ofTotal: 'of {count} total',
        playersHint: '{count} players on the platform',
        arr: 'ARR',
        arrHint: 'Annual projection (MRR × 12)',
        overdueCount: '{count} overdue invoices',
        kpis: {
          activeAcademies: 'Active academies',
          totalUsers: 'Total users',
          activePlans: 'Active plans',
        },
        quickLinks: 'Quick access',
        openLink: 'Open →',
        quickLinksPlans: 'Manage plans',
        quickLinksPlansDesc: 'Configure limits, pricing, and billing cycles.',
        quickLinksAcademies: 'Manage academies',
        quickLinksAcademiesDesc: 'Tenants, plans, and access.',
        quickLinksSuperAdmins: 'Super administrators',
        quickLinksSuperAdminsDesc: 'Global platform access.',
        quickLinksBilling: 'Billing',
        quickLinksBillingDesc: 'Invoices, payments, and overdue items.',
        quickLinksAudit: 'Audit log',
        quickLinksAuditDesc: 'Platform action history.',
        mrr: 'MRR',
        mrrHint: 'Monthly recurring revenue (active academies)',
        delinquency: 'Delinquency',
        delinquencyHint: 'Overdue / issued this month',
        billingMonth: 'Billing this month',
        billed: 'Billed',
        collected: 'Collected',
        pendingCollection: 'Pending collection',
        academyBreakdown: 'Academies',
        suspendedBilling: 'Suspended for billing',
        suspendedManual: 'Manually suspended',
        newAcademies: 'New this month',
        usersByRole: 'Users by role',
        attentionTitle: 'Needs your attention',
        attentionEmpty: 'All clear. No pending alerts.',
        attentionSuspended: 'Academies with overdue billing',
        attentionOverdue: 'Overdue invoices',
        viewAcademy: 'Review',
        viewBilling: 'Review',
        viewInvoice: 'View invoice',
        chartGrowth: 'Academy growth',
        chartRevenue: 'Monthly revenue',
        deltaVsPrevious: '{value} vs previous month',
        deltaNewAcademies: '{count} new ({delta} vs previous month)',
      },
    },
    academyAdmin: {
      greeting: 'Hello, {role}',
      greetingMulti: 'Unified dashboard ({roles})',
      description: 'Your academy administration dashboard.',
      welcome:
        'Welcome to the academy dashboard. From here you will manage players, categories, coaches, and matches.',
      billing: {
        title: 'Billing',
        description: 'Your plan and billing.',
      },
      users: {
        title: 'Users',
        description: 'The people who make the academy run.',
      },
      categories: {
        title: 'Categories',
        description: 'How you organize your teams.',
      },
      players: {
        title: 'Players',
        description: 'The stars of your academy.',
      },
      actions: {
        title: 'Action catalog',
        description: 'Customize what gets tracked in every match.',
      },
      matches: {
        title: 'Matches',
        description: 'The season calendar.',
      },
      home: {
        title: 'Home dashboard',
        description: 'Operational pulse of your academy.',
        academyLabel: 'Your academy',
        pulseSubtitle: 'Overview of players, categories, matches, and billing.',
        unassignedCategory: 'Unassigned',
        activePlayers: 'Active players',
        chartPlayersByCategory: 'Active players by category',
        billingStatus: 'Billing status',
        nextPeriodEnd: 'Next period end',
        matchesInProgress: 'Matches in progress',
        suspensionWarning:
          'An overdue invoice may suspend access for the entire academy. Review billing soon.',
        quickLinksTitle: 'Quick links',
        attentionTitle: 'Needs your attention',
        attentionEmpty: 'All caught up. No pending actions.',
        attentionAction: 'Go resolve',
        withoutCoach: '{count} without coach',
        playersLimit: '{current} / {limit} plan limit',
        usersBreakdown: '{coaches} coaches · {parents} parents · {admins} admins',
        inProgressMatches: '{count} in progress now',
        nextDue: 'Next due: {date}',
        kpis: {
          activePlayers: 'Active players',
          pendingPlayers: 'Pending approval',
          categories: 'Categories',
          users: 'Users',
          upcomingMatches: 'Upcoming matches',
          billing: 'Current plan',
        },
        billingStatuses: {
          current: 'Current',
          pending: 'Payment pending',
          overdue: 'Overdue',
        },
        attention: {
          pendingPlayers: '{count} players pending approval',
          pendingPlayersHint: 'Parents enrolled players waiting for your review.',
          categoriesWithoutCoach: '{count} categories without coach',
          categoriesWithoutCoachHint: 'Assign a coach so they can manage matches.',
          billingOverdue: 'Overdue invoice',
          billingOverdueHint: 'Settle payment to avoid academy suspension.',
          billingPending: 'Invoice pending payment',
          billingPendingHint: 'You have an upcoming or pending charge in billing.',
          upcomingMatches: '{count} matches in the coming days',
        },
      },
      settings: {
        title: 'My academy',
        description: 'Configure your academy settings.',
      },
      reports: {
        title: 'Reports',
        description: 'Your academy data.',
      },
    },
    coach: {
      greeting: 'Hello, {role}',
      greetingMulti: 'Unified dashboard ({roles})',
      description: 'Your pulse on the pitch.',
      welcome:
        'Welcome, coach. Here you will capture live actions and manage your assigned categories.',
      home: {
        title: 'Home',
        description: 'Your pulse on the pitch.',
        heroEyebrow: 'Coach',
        heroHello: 'Hi, {name}',
        heroSubtitle: 'Prep the next match and keep up with your categories.',
        errors: {
          title: 'Error',
        },
        kpis: {
          categories: 'My categories',
          upcoming: 'Upcoming matches',
          playedMonth: 'Played this month',
          inProgress: 'Live now',
        },
        nextMatchLabel: 'Next match',
        inDays: 'In {days} days',
        prepareMatch: 'Prepare match',
        continueCapture: 'Continue capture',
        attentionTitle: 'Needs your attention',
        attentionEmpty: 'All clear. Nothing pending right now.',
        attentionAction: 'Go',
        openLink: 'Open',
        attention: {
          liveMatch: 'Live match vs {opponent}',
          upcomingSoon: '{count} matches in the next few days',
        },
      },
      categories: {
        title: 'My categories',
        description: 'The teams you coach.',
        empty: 'You have no assigned categories yet.',
        playerCount: '{count} players',
        matchCount: '{count} matches',
        viewPlayers: 'View players',
      },
      players: {
        title: 'My players',
        description: 'The talents you shape.',
        empty: 'No players in your matches yet.',
        emptyFiltered: 'No players in this category.',
        viewMatches: 'View matches',
        filterAll: 'All',
        jersey: 'Jersey',
      },
      matches: {
        title: 'My matches',
        description: 'Your season calendar.',
        detailDescription: 'Every match, summarized.',
      },
      analysis: {
        title: 'Player analysis',
        description: 'Compare, filter, and discover patterns in your squad.',
        heroEyebrow: 'Analysis',
        subtitle: 'Compare, filter, and discover patterns in your squad.',
        feedback: 'Showing {players} players · {actions} recorded actions · {matches} matches in range',
        exportCsv: 'Export CSV',
        exportPdf: 'Export PDF',
        exporting: 'Exporting…',
        exportSuccess: 'CSV downloaded',
        exportPdfSuccess: 'PDF downloaded',
        viewCards: 'Cards',
        viewTable: 'Table',
        emptyTitle: 'No players to analyze',
        emptyDescription: 'When you have players in your categories, their stats will show up here.',
        sortAsc: 'Sort ascending',
        sortDesc: 'Sort descending',
        prevPage: 'Previous',
        nextPage: 'Next',
        pageOf: 'Page {page} of {pages}',
        backToCompare: 'Back to comparison',
        filters: {
          title: 'Filters',
          category: 'Category',
          allCategories: 'All my categories',
          match: 'Match',
          allMatches: 'All matches in period',
          dateFrom: 'From',
          dateTo: 'To',
          action: 'Action',
          allActions: 'All',
          impact: 'Impact',
          impactAll: 'All',
          impactPositive: 'Positive',
          impactNegative: 'Negative',
          impactNeutral: 'Neutral',
          thisMonth: 'This month',
          last30: 'Last 30 days',
          last90: 'Last 90 days',
          fullSeason: 'Full season',
          clear: 'Clear filters',
        },
        chart: {
          topTotal: 'Top 5 players by total actions',
          topPositive: 'Top 5 players by positive actions',
          topNegative: 'Top 5 players by negative actions',
          topNeutral: 'Top 5 players by neutral actions',
          topAction: 'Top 5 in {action}',
        },
        noActions: 'No actions in this period',
        actionChipsLabel: 'Player actions',
        cols: {
          player: 'Player',
          category: 'Category',
          matches: 'Matches',
          minutes: 'Minutes',
          actions: 'Actions',
          positive: 'Positive',
          negative: 'Negative',
          neutral: 'Neutral',
          observations: 'Observations',
          breakdown: 'Breakdown',
        },
        detail: {
          title: 'Player detail',
          description: 'How they shine in every match.',
          topAction: 'Top: {action}',
          topActionEmpty: 'No top action',
          evolution: 'Monthly evolution',
          radar: 'Dimension radar',
          actionsBreakdown: 'Actions breakdown',
          matchesTitle: 'Matches in filter',
          matchesEmpty: 'No matches played with these filters.',
          rival: 'Opponent',
          date: 'Date',
          viewReportCard: 'View match card',
          observationsTitle: 'Observations',
          observationsEmpty: 'No observations in this period.',
        },
        errors: {
          title: 'Could not load analysis',
          generic: 'Something went wrong loading analysis. Try again.',
          export: 'Could not export the file. Check your connection.',
        },
      },
    },
    parent: {
      greeting: 'Hello, {role}',
      greetingMulti: 'Unified dashboard ({roles})',
      description: 'Follow your children’s development at the academy.',
      welcome: 'Welcome. Here we celebrate every step your children take on the pitch.',
      home: {
        title: 'Home',
        description: 'Follow your children’s development.',
        heroEyebrow: 'Family',
        heroHello: 'Hi, {name}',
        heroSubtitle: 'Follow your children’s development at the academy.',
        captureMatches: 'Capture live match',
      },
      children: {
        title: 'My children',
        description: 'The talents in development.',
      },
      calendar: {
        title: 'Match calendar',
        description: "Your family's upcoming matches.",
      },
      notifications: {
        title: 'Notifications',
        description: 'Choose what alerts you want to receive.',
      },
    },
    player: {
      greeting: 'Hello, {role}',
      greetingMulti: 'Unified dashboard ({roles})',
      description: 'Your development at the academy.',
      welcome: 'Welcome. Here you can see your profile, matches and reports.',
      home: {
        title: 'Home',
        description: 'Your performance overview.',
        heroEyebrow: 'Player',
        heroHello: 'Hi, {name}',
        heroSubtitle: 'Follow your progress at the academy.',
        goProfile: 'View my profile',
        goMatches: 'View my matches',
        goReports: 'View my reports',
      },
      profile: {
        title: 'My profile',
        description: 'Your player details.',
        firstName: 'First name',
        lastName: 'Last name',
        jersey: 'Jersey',
        category: 'Category',
        position: 'Position',
        dateOfBirth: 'Date of birth',
        changePhoto: 'Change my photo',
        photoHint: 'You and the academy team can see your photo.',
        editSection: 'My details',
        editHint:
          'You can update your name, date of birth and position. Jersey and category are set by the academy.',
        save: 'Save my details',
        successUpdate: 'Profile updated',
        readOnlyAssigned: 'Assigned by the academy',
      },
      matches: {
        title: 'My matches',
        description: 'Match history and report cards.',
        emptyTitle: 'No match reports yet',
        emptyDescription:
          'When you play matches with captured actions, your report cards will appear here.',
        opponent: 'Opponent',
        date: 'Date',
        actions: 'Actions',
      },
      calendar: {
        title: 'My calendar',
        description: 'Your upcoming matches.',
        upcomingSection: 'Upcoming',
        pastSection: 'Recent',
        noUpcoming: 'You have no scheduled matches right now.',
        emptyTitle: 'Nothing on your calendar',
        emptyDescription:
          'When your academy schedules matches for your category, they will show up here.',
        vsOpponent: 'vs {opponent}',
        location: '{location}',
        today: 'Today',
        tomorrow: 'Tomorrow',
        inDays: 'In {count} days',
        inProgress: 'Live',
        viewReportCard: 'View my report card',
      },
      reports: {
        title: 'My reports',
        description: 'Notes from your coaches.',
        intro:
          'Comments from your coaches about your progress. Each note reflects the academy’s support.',
        emptyTitle: 'No observations yet',
        emptyDescription: 'When your coach shares comments about you, you will see them here.',
      },
      dashboard: {
        kpiMatches: 'Matches played',
        kpiMinutes: 'Total minutes',
        kpiActions: 'Recorded actions',
        highlights: 'My highlights',
        emptyTitle: 'No data yet',
        emptyDescription:
          'When you play matches with captured actions, your progress will appear here.',
        viewReportCard: 'View my report card',
      },
      errors: {
        title: 'We could not load your data',
        generic: 'Check your connection and try again.',
      },
    },
  },
  parent: {
    errors: {
      title: 'Error',
      generic: 'We could not complete the operation. Please try again.',
    },
    children: {
      name: 'Name',
      firstName: 'First name',
      lastName: 'Last name',
      dateOfBirth: 'Date of birth',
      position: 'Position',
      category: 'Category',
      jersey: 'Jersey',
      statusColumn: 'Status',
      enroll: 'Enroll child',
      enrollTitle: 'Enroll child',
      enrollSubmit: 'Submit enrollment',
      editTitle: 'Edit details',
      enrollNoticeTitle: 'Academy review',
      enrollNotice:
        'Your enrollment will remain pending until the academy approves it.',
      pendingMessage: 'Pending approval by the academy.',
      pendingBannerTitle: 'Enrollments under review',
      pendingBanner: 'You have {count} enrollment(s) waiting for academy approval.',
      rejectedMessage: 'Enrollment rejected: {reason}',
      empty: 'You have no enrolled children yet.',
      emptyDescription: 'Enroll your child by selecting an academy category.',
      successEnroll: 'Enrollment submitted. The academy will review it soon.',
      successUpdate: 'Details updated',
      status: {
        active: 'Active',
        pendingApproval: 'Pending approval',
        rejected: 'Rejected',
      },
      viewReportCards: 'View match report cards',
    },
  },
  parentDashboard: {
    periodLabel: 'Period',
    periodAll: 'Full season',
    childTabs: 'Select child',
    tabLabel: '{name} · #{jersey}',
    kpiMatches: 'Matches played',
    kpiMinutes: 'Total minutes',
    kpiActions: 'Recorded actions',
    highlights: 'Top actions',
    chartTitle: 'Monthly trend',
    chartActions: 'Actions',
    byMatchTitle: 'By match',
    byMonthTitle: 'By month',
    actionColumn: 'Action',
    totalColumn: 'Total',
    viewReportCard: 'View match report card',
    emptyDashboard: 'No data for this period',
    emptyDashboardDescription:
      'When your child plays matches with captured actions, their progress will appear here.',
    emptyTable: 'No actions recorded in this period.',
  },
  parentCalendar: {
    title: 'Match calendar',
    subtitle: "Your family's upcoming matches.",
    upcomingSection: 'Upcoming',
    pastSection: 'Recent',
    noUpcoming: 'No matches scheduled yet. Your academy will let you know when there is news.',
    emptyTitle: 'No matches on the calendar',
    emptyDescription:
      'No matches scheduled yet. Your academy will let you know when there is news.',
    vsOpponent: 'vs {opponent}',
    location: '{location}',
    playerLabel: 'Called up: {name} · #{jersey}',
    today: 'Today',
    tomorrow: 'Tomorrow',
    inDays: 'In {count} days',
    inProgress: 'In progress now',
    viewReportCard: 'View report card',
  },
  parentNotifications: {
    bellLabel: 'Notifications ({count} unread)',
    bellLabelNone: 'Notifications',
    panelTitle: 'Your notifications',
    markAllRead: 'Mark all read',
    markRead: 'Mark read',
    viewMatch: 'View match',
    empty: 'No notifications yet. When your child has a highlight on the pitch, we will let you know here.',
    voidedHint: 'This action was corrected by the coach.',
    managePreferences: 'Notification preferences',
    preferences: {
      globalTitle: 'In-app notifications',
      globalDescription:
        'Get alerts when your child shines in a match (goal, assist, and more).',
      inApp: 'Receive in-app notifications',
      emailComingSoon: 'Email notifications will be available soon.',
      byChildTitle: 'Per child',
      byChildDescription: 'Turn alerts on or off for each linked child.',
      inAppFor: 'Notifications for {name}',
    },
  },
  notifications: {
    messages: {
      goal: {
        title: 'Goal!',
        body: '{playerName} scored a goal in minute {minute}!',
      },
      assist: {
        title: 'Assist!',
        body: '{playerName} assisted a goal in minute {minute}!',
      },
      shotOnTarget: {
        title: 'Shot on target',
        body: '{playerName} had a shot on target in minute {minute}.',
      },
      interception: {
        title: 'Interception',
        body: '{playerName} intercepted a pass in minute {minute}.',
      },
      tackle: {
        title: 'Great tackle',
        body: '{playerName} won the ball with a tackle in minute {minute}.',
      },
      ballRecovery: {
        title: 'Ball recovery',
        body: '{playerName} recovered the ball in minute {minute}.',
      },
      save: {
        title: 'Save!',
        body: '{playerName} made a save in minute {minute}!',
      },
      highlight: {
        title: 'Highlight moment!',
        body: '{playerName} had a highlight: {actionName} (min {minute})!',
      },
    },
  },
  playerObservations: {
    title: 'Coach observations',
    parentIntro:
      'Messages from coaches about your child’s progress. Each note reflects the academy’s support in their development.',
    playerIntro:
      'Comments from your coaches about your progress. Each note reflects the academy’s support.',
    typeGeneral: 'General observation',
    typeMatch: 'About a match',
    sectionGeneral: 'General observations',
    sectionMatch: 'By match',
    formLabel: 'Your observation',
    formPlaceholder: 'Write constructive feedback about the player…',
    scopeGeneral: 'General observation (not linked to a match)',
    submit: 'Publish observation',
    delete: 'Delete',
    confirmDelete: 'Delete this observation?',
    selectPlayer: 'Player',
    noPlayersPresent: 'Mark attendance to add player observations.',
    emptyTitle: 'No observations yet',
    emptyCoach: 'Observations you publish will appear here for the player’s family.',
    emptyParent:
      'When the coach shares comments about your child, you will see them here.',
    emptyPlayer: 'When your coach shares comments about you, you will see them here.',
    errors: {
      title: 'Error',
      generic: 'We could not load observations. Please try again.',
    },
  },
  reportCard: {
    back: 'Back',
    backToChildren: 'Back to my children',
    backToMatches: 'Back to my matches',
    viewCard: 'View report card',
    share: 'Share report card',
    shareTitle: '{name} — match report',
    shareCopied: 'Link copied to clipboard',
    shareError: 'Could not copy the link',
    chartToggleLabel: 'Chart type',
    chartRadar: 'Radar',
    chartBars: 'Bars',
    scoreLabel: 'Performance',
    noActions: 'No actions recorded in this match yet.',
    noActionsShort: 'No actions',
    noDimensions: 'No dimensions with data to display.',
    minutesPlayed: 'Minutes played',
    minutesUnit: 'min',
    perMinuteShort: 'min',
    totalActions: 'Total actions',
    actionCount: '{count} actions',
    opponent: 'Opponent',
    date: 'Date',
    actions: 'Actions',
    emptyMatches: 'No finished matches',
    emptyMatchesDescription:
      'When your child plays matches and the coach records actions, their report cards will appear here.',
    matchesListTitle: 'Match report cards',
    matchesListDescription: 'Finished matches with captured performance data.',
    errors: {
      title: 'Error',
      generic: 'We could not load the report card. Please try again.',
      notFound: 'Report card not found',
    },
    dimensions: {
      attack: 'Attack',
      creation: 'Creation',
      defense: 'Defense',
      recovery: 'Recovery',
      goalkeeping: 'Goalkeeping',
      discipline: 'Discipline',
    },
    motivation: {
      default: 'Great match! Every minute on the pitch builds experience.',
      attack: 'Pure firepower in attack! You made the difference up front today.',
      creation: 'Pure creativity! Your passes and dribbles opened up the game.',
      defense: 'You dominated defensively today! A wall on the pitch.',
      recovery: 'Unstoppable engine! You won the ball back again and again.',
      goalkeeping: 'Golden hands! You protected the goal like a pro.',
      discipline: 'Clean and focused play! Great attitude on the pitch.',
    },
    insight: {
      title: 'AI analysis',
      description: 'Interprets this match\'s stats and suggests what to improve and how, in your own language.',
      generate: 'Generate AI analysis',
      generating: 'Generating...',
      generatingHint: 'The first time can take up to half a minute.',
      regenerate: 'Regenerate',
      generatedAt: 'Generated on {date}',
      errorGeneric: 'We could not generate the analysis. Try again.',
      errorTimeout: 'The analysis is taking longer than expected. Try again in a moment.',
    },
  },
  tenant: {
    errors: {
      title: 'Error',
      generic: 'We could not complete the operation. Please try again.',
    },
    filters: {
      all: 'All',
    },
    autocomplete: {
      placeholder: 'Search by name or email…',
      noResults: 'No results',
      selected: 'Selected',
      remove: 'Remove {label}',
      searchParents: 'Search parents/guardians…',
      searchPlayers: 'Search players…',
    },
    users: {
      email: 'Email',
      firstName: 'First name',
      lastName: 'Last name',
      role: 'Role',
      rolesLabel: 'Roles',
      rolesHint: 'Select one or more roles. The user will access sections for all of them.',
      primaryRole: 'primary',
      lastRoleWarning: 'At least one role is required. To revoke access, deactivate the user.',
      status: 'Status',
      lastLogin: 'Last login',
      lastLoginNever: 'Never',
      searchPlaceholder: 'Search by email or name…',
      filterRole: 'Role',
      filterStatus: 'Status',
      empty: 'No users registered in your academy.',
      createSubmit: 'Create user',
      createTitle: 'New user',
      editTitle: 'Edit user',
      edit: 'Edit',
      resetPassword: 'Reset password',
      successCreate: 'User created successfully',
      successUpdate: 'User updated',
      successStatus: 'Status updated',
      activate: 'Activate',
      deactivate: 'Deactivate',
      tempPasswordTitle: 'Temporary password',
      tempPasswordDescription: 'Share this password with the user. They should change it on first login.',
      linkedPlayers: 'Linked players',
      linkedPlayersHint: 'Link existing players or register a new active player.',
      createChild: 'Register new child',
      createChildSubmit: 'Create and link',
      successCreateChild: 'Player created and linked',
      saveFirstToLink: 'Save the user first to link players.',
      kpis: {
        total: 'Plan users',
        limit: 'Limit: {limit}',
      },
    },
    categories: {
      name: 'Name',
      ageRange: 'Age range',
      ageMin: 'Min age',
      ageMax: 'Max age',
      coach: 'Coach',
      noCoach: 'No coach',
      status: 'Status',
      searchPlaceholder: 'Search category…',
      filterStatus: 'Status',
      empty: 'No categories yet.',
      create: 'New category',
      createTitle: 'Create category',
      editTitle: 'Edit category',
      successCreate: 'Category created',
      successUpdate: 'Category updated',
      successStatus: 'Category status updated',
      activate: 'Activate',
      deactivate: 'Deactivate',
      requiresGuardian: {
        label: 'Guardian / adult player',
        auto: 'Automatic (by age)',
        required: 'Requires a guardian',
        adult: 'Adult player (no guardian)',
        hint: 'Automatic follows age: if max age is under 18 or unset, a guardian is required. “Adult player” enables the invite even when max age is under 18.',
      },
      kpis: {
        total: 'Categories',
        limit: 'Limit: {limit}',
        withCoach: 'With coach',
        withoutCoach: 'Without coach',
      },
    },
    players: {
      name: 'Name',
      firstName: 'First name',
      lastName: 'Last name',
      dateOfBirth: 'Date of birth',
      jersey: 'Jersey #',
      position: 'Position',
      category: 'Category',
      noCategory: 'No category',
      parents: 'Linked parents',
      noParents: 'No parents',
      searchParents: 'Search parents/guardians…',
      noParentsAvailable: 'Create parent users to link them here.',
      statusColumn: 'Status',
      searchPlaceholder: 'Search player…',
      filterStatus: 'Status',
      filterCategory: 'Category',
      empty: 'No players registered.',
      create: 'New player',
      createTitle: 'Register player',
      editTitle: 'Edit player',
      successCreate: 'Player registered',
      successUpdate: 'Player updated',
      successActivate: 'Player activated',
      successDeactivate: 'Player deactivated',
      successReactivate: 'Player reactivated',
      successDelete: 'Player deleted',
      successApprove: 'Enrollment approved',
      successReject: 'Enrollment rejected',
      approve: 'Approve',
      reject: 'Reject',
      deactivate: 'Deactivate',
      reactivate: 'Reactivate',
      remove: 'Delete',
      deactivateConfirmTitle: 'Deactivate {name}?',
      deactivateConfirmDescription:
        'They will no longer appear in new matches or count toward the next invoice. Their history is kept and linked parents can still view their information. You can reactivate them later.',
      reactivateConfirmTitle: 'Reactivate {name}?',
      reactivateConfirmDescription:
        'They will count again as an active plan player and appear in attendance and capture for new matches.',
      deleteConfirmTitle: 'Delete {name}?',
      deleteConfirmDescription:
        'This cannot be undone. Only allowed if the player has no match history. Parent links will also be removed.',
      deactivatedOn: 'Deactivated on {date}',
      rejectedWithReason: 'Rejected: {reason}',
      rejectedNoReason: 'Enrollment rejected',
      approveTitle: 'Approve enrollment',
      approveDescription: 'Confirm activation for {name}. You may adjust category and jersey number.',
      rejectTitle: 'Reject enrollment',
      rejectReason: 'Reason (optional)',
      rejectReasonPlaceholder: 'E.g. age outside category range',
      pendingBannerTitle: 'Pending enrollments',
      pendingBanner: '{count} player(s) awaiting your approval.',
      viewPending: 'View pending',
      activate: 'Approve',
      kpis: {
        active: 'Active players',
        limit: 'Limit: {limit}',
        pending: 'Pending approval',
        categories: 'Categories with players',
      },
      status: {
        active: 'Active',
        inactive: 'Inactive',
        pending: 'Pending',
        injured: 'Injured',
        retired: 'Retired',
      },
      adultInvite: {
        toggle: 'This player manages their own account (adult player)',
        saveFirst: 'Save the player first, then you can invite them as an adult.',
        alreadyInvited: 'This player already has an access account. Another invite will not be created.',
        email: 'Access email',
        emailPlaceholder: 'email@example.com',
        emailRequired: 'Enter an email to create the adult player account.',
        success: 'Invite sent. Share the temporary password.',
        categoryRequiresGuardian:
          'Cannot invite as an adult: this category requires a guardian. Change the category setting or pick another category.',
        tempPasswordTitle: 'Adult player account',
        tempPasswordDescription:
          'Share this email and the temporary password. The player must change it on first sign-in.',
      },
    },
    actionCatalog: {
      code: 'Code',
      name: 'Name',
      description: 'Description',
      impactLabel: 'Impact',
      notifiableLabel: 'Notifiable',
      notifiableToggle: 'Notify parent/guardian when this action occurs',
      notifiableYes: 'Yes',
      notifiableNo: 'No',
      status: 'Status',
      searchPlaceholder: 'Search by code or name…',
      filterImpact: 'Impact',
      filterImpactAll: 'All impacts',
      filterStatus: 'Status',
      filterStatusAll: 'All statuses',
      empty: 'No actions in the catalog yet.',
      create: 'New action',
      createTitle: 'Create action',
      editTitle: 'Edit action',
      remove: 'Remove',
      deleteConfirm: 'Remove action «{name}»? Only allowed if it was never used in matches.',
      successCreate: 'Action created',
      successUpdate: 'Action updated',
      successStatus: 'Action status updated',
      successDelete: 'Action removed',
      activate: 'Activate',
      deactivate: 'Deactivate',
      usedBadge: 'Used in matches',
      usedHint: 'This action was recorded in matches; it cannot be deleted.',
      usedWarningTitle: 'Action with history',
      usedWarningBody:
        'This action was used in capture. You may edit name, description, and notifiable flag, but not code or impact (they protect historical interpretation).',
      impact: {
        positive: 'Positive',
        negative: 'Negative',
        neutral: 'Neutral',
      },
      kpis: {
        active: 'Active actions',
        notifiable: 'Notifiable',
        byImpact: 'Pos / Neg / Neu',
        byImpactHint: 'Active by impact',
      },
    },
  },
  matches: {
    errors: {
      title: 'Error',
      generic: 'We could not complete the operation. Please try again.',
      notFound: 'Match not found.',
    },
    opponent: 'Opponent',
    category: 'Category',
    datetime: 'Date & time',
    location: 'Location',
    notes: 'Notes',
    noLocation: 'No location set',
    typeColumn: 'Type',
    statusColumn: 'Status',
    searchPlaceholder: 'Search match…',
    filterStatus: 'Status',
    filterCategory: 'Category',
    selectCategory: 'Select category',
    empty: 'No matches scheduled.',
    create: 'New match',
    createTitle: 'Schedule match',
    editTitle: 'Edit match',
    viewDetail: 'View details',
    continueCapture: 'Continue capture',
    inDays: 'In {days} days',
    backToList: 'Back to list',
    successCreate: 'Match created',
    successUpdate: 'Match updated',
    successStatus: 'Status updated',
    successCancel: 'Match cancelled',
    cancelConfirmTitle: 'Cancel match?',
    cancelConfirmBody: 'This action cannot be undone. The match will be marked as cancelled.',
    periods: 'Game periods',
    periodsSummary: '{count} periods × {minutes} min ({source})',
    periodsFromAcademy: 'academy default',
    periodsCustom: 'custom',
    createdBy: 'Created by',
    validation: {
      categoryRequired: 'Select a category',
      opponentRequired: 'Opponent is required',
      datetimeRequired: 'Date and time are required',
    },
    actions: {
      start: 'Start match',
      finish: 'Finish',
      cancel: 'Cancel match',
    },
    kpis: {
      upcoming: 'Upcoming',
      inProgress: 'In progress',
      playedMonth: 'Played this month',
    },
    type: {
      league: 'League',
      friendly: 'Friendly',
      tournament: 'Tournament',
    },
    status: {
      scheduled: 'Scheduled',
      in_progress: 'In progress',
      finished: 'Finished',
      cancelled: 'Cancelled',
    },
    tabs: {
      label: 'Match sections',
      overview: 'Overview',
      attendance: 'Attendance',
      capture: 'Live capture',
      observations: 'Observations',
      comingSoon: 'coming soon',
      attendanceHint: 'Attendance management will be available in the next step.',
      captureHint: 'The live capture board will be available in the next step.',
    },
    attendance: {
      kpiPresent: 'Present',
      kpiStarters: 'Starters',
      kpiSubstitutes: 'Substitutes',
      present: 'Present',
      absent: 'Absent',
      starter: 'Starter',
      substitute: 'Substitute',
      lineupLabel: 'Lineup role',
      jerseyLabel: 'Match jersey',
      defaultJersey: 'Profile: #{number}',
      inactivePlayer: 'inactive',
      save: 'Save attendance',
      saving: 'Saving…',
      successSave: 'Attendance saved',
      unsavedHint: 'You have unsaved changes',
      markAllPresent: 'Mark all present',
      readOnlyHint: 'Only the assigned coach for this category can edit.',
      lockedHint: 'This match is finished or cancelled; attendance cannot be edited.',
      emptyTitle: 'No active players',
      emptyDescription: 'There are no active players in this match category.',
      collisionTitle: 'Duplicate jerseys',
      jerseyCollision: 'Jerseys {jerseys} are duplicated among present players.',
      jerseyCollisionServer: 'Duplicate jerseys detected. Fix the numbers before saving.',
      forbiddenCategory:
        'You are not the coach assigned to this match category. Only that category coach can view and mark attendance.',
      serverError:
        'We could not load attendance. If you just updated the project, run the migration (pnpm db:migrate) and tap Retry.',
      viewReportCard: 'View match report card',
    },
    capture: {
      coachOnly: 'coach only',
      forbiddenTitle: 'No capture access',
      forbiddenBody: 'Only the coach assigned to this match category can capture actions.',
      scheduledHint: 'Mark attendance and start the match to open live capture.',
      cancelledHint: 'This match was cancelled; capture is unavailable.',
      readOnlyHint: 'Match finished. Live capture is disabled; you can review the action history.',
      correctionModeTitle: 'Correction mode',
      correctionModeBanner:
        'Correction mode — {days} day(s) left to edit this match. New actions are marked as post-match corrections.',
      correctionClosedHint:
        'Match closed — the 7-day correction period has ended. History is read-only.',
      correctionBadge: 'Correction',
      correctionTwoTapHint:
        'Post-match correction: tap player and action to add. Use void in history to correct existing actions.',
      devReopen: 'Reopen for testing (dev)',
      devReopenHint: 'Development only: sets match back to in progress without deleting actions.',
      devReopenSuccess: 'Match reopened for testing',
      noPlayersTitle: 'No attendees',
      noPlayersBody: 'Mark present players with jerseys in the Attendance tab before capturing.',
      period: 'Period',
      minute: 'Minute',
      minuteIncrease: 'Increase minute',
      minuteDecrease: 'Decrease minute',
      clockDisplay: 'P{period} · {minute}\'',
      clockRunning: 'Running',
      clockPaused: 'Paused',
      clockPause: 'Pause',
      clockResume: 'Resume',
      clockNextPeriod: 'Next period',
      clockNextPeriodShort: 'P{next}/{total}',
      clockLastPeriod: 'Last period of the match',
      clockAdjust: 'Adjust',
      clockAdjustTitle: 'Adjust minute',
      clockAdjustHint: 'Sync with the referee if the clock drifted.',
      clockAdjustConfirm: 'Apply',
      playersSection: 'Present players',
      actionsSection: 'Actions',
      boardFullscreenLabel: 'Capture board',
      enterBoardFullscreen: 'View fullscreen',
      exitBoardFullscreen: 'Exit fullscreen',
      twoTapHint: 'Tap a player and an action in any order to record instantly.',
      voiceSoon: 'Voice capture — coming soon',
      voiceCapture: {
        title: 'Voice capture',
        hint: 'Say jersey + action in a row ("seven goal", "ten recovery"). Enable continuous mode to keep the mic open. Say "undo" or "no, it was eight" to correct.',
        listening: 'Listening',
        listeningContinuous: 'Continuous listen',
        stopped: 'Stopped',
        startListening: 'Start listening',
        stopListening: 'Stop listening',
        continuousMode: 'Continuous mode (mic always on)',
        continuousActiveHint: 'Mic is open — say actions back-to-back. Turn off continuous mode to save battery.',
        soundFeedback: 'Sound on register',
        vibrationFeedback: 'Vibration on register',
        notSupported: 'Not supported',
        notSupportedDetail:
          'Your browser does not support speech recognition. Try Chrome on Android or Chrome/Edge on desktop.',
        permissionDenied: 'Permission denied',
        permissionDeniedDetail:
          'Microphone access is required. Enable it in browser settings and try again.',
        insecureContext:
          'The microphone requires HTTPS or localhost. On local network (IP) the browser may block it.',
        error: 'Error',
        errorDetail: 'Could not listen ({code}). Check connection or microphone.',
        interimLabel: 'Forming…',
        lastLabel: 'Last phrase',
        empty: 'Say something, e.g. "seven goal"…',
        historyLabel: 'Recent phrases',
        lang: 'Language: {lang}',
        confirmBeforeRegister: 'Confirm before registering by voice',
        confirmPrompt: '#{jersey} {lastName} — {action}?',
        confirmHeard: 'Heard: "{text}"',
        confirmAction: 'Confirm record',
        confirmVoiceHint: 'You can also say "yes" or tap Confirm.',
        correctPrompt: 'Correct to #{jersey} {lastName} — {action}?',
        registered: 'Recorded #{jersey} · {action}',
        cancelled: 'Voice registration cancelled',
        undoSuccess: 'Last action undone',
        simulateLabel: 'Test with text',
        simulatePlaceholder: 'e.g. seven goal',
        simulateSubmit: 'Test',
        errors: {
          empty: 'I did not understand. Try again.',
          noJersey: 'I did not catch the jersey number. Say the player number.',
          noPlayer: 'No player #{jersey} on the pitch.',
          noAction: 'I did not understand the action. Try again.',
          ambiguous: 'Several actions match. Be more specific or confirm.',
          generic: 'Could not record by voice. Try again or use the touch board.',
          noUndoTarget: 'No recent action to undo.',
          undoExpired: 'Immediate undo window expired. Use void to correct.',
          undoFailed: 'Could not undo the action.',
          noCorrectTarget: 'No recent action to correct.',
        },
      },
      voiceExperiment: {
        title: 'Voice experiment',
        hint: 'Phase 1: listen and show text only. Does not record actions.',
        listening: 'Listening',
        stopped: 'Stopped',
        startListening: 'Start listening',
        stopListening: 'Stop listening',
        notSupported: 'Not supported',
        notSupportedDetail:
          'Your browser does not support speech recognition. Try Chrome on Android or Chrome/Edge on desktop.',
        permissionDenied: 'Permission denied',
        permissionDeniedDetail:
          'Microphone access is required. Enable it in browser settings and try again.',
        insecureContext:
          'The microphone requires HTTPS or localhost. On local network (IP) the browser may block it.',
        error: 'Error',
        errorDetail: 'Could not listen ({code}). Check connection or microphone.',
        interimLabel: 'Forming…',
        lastLabel: 'Last phrase',
        empty: 'Say something, e.g. "seven goal"…',
        historyLabel: 'Recent phrases (test)',
        lang: 'Language: {lang}',
      },
      history: 'History',
      historyOpen: 'View',
      historyEmpty: 'No actions recorded yet.',
      recordingFor: 'Recording for #{jersey} {lastName}',
      selectAction: 'Choose an action',
      playerPickHint: 'Tap a player to record an action.',
      periodMinute: 'P{period} · {minute}\'',
      toastRecorded: 'Recorded: #{jersey} · {action}',
      undo: 'Undo',
      undoBanner: 'Finger slip? You can undo the last action.',
      undoSuccess: 'Action undone',
      undoFailed: 'Could not undo. Use void if the window expired.',
      retry: 'Retry',
      void: 'Void',
      voided: 'Voided',
      voidTitle: 'Void action',
      voidDescription: 'The action stays visible as voided. Optional reason below.',
      voidReasonPlaceholder: 'Reason (optional)',
      voidConfirm: 'Void action',
      voidSuccess: 'Action voided',
      voidFailed: 'Could not void the action',
      finishMatch: 'Finish match',
      finishConfirmTitle: 'Finish match?',
      finishConfirmBody:
        'Live capture will stop. You will have 7 days for post-match corrections (void or add actions).',
      sendStatus: {
        sending: 'Sending',
        confirmed: 'Confirmed',
        failed: 'Send failed',
      },
    },
  },
  reports: {
    hint: 'CSV files use semicolon (;) and UTF-8 encoding so they open correctly in Excel. PDFs include your academy logo and name.',
    exportCsv: 'Export CSV',
    exportPdf: 'Export PDF',
    exportSuccess: 'Download started',
    exportError: 'We could not generate the report. Please try again.',
    filters: {
      category: 'Category',
      allCategories: 'All categories',
      status: 'Status',
      allStatuses: 'All statuses',
      role: 'Role',
      allRoles: 'All roles',
      matchType: 'Match type',
      allTypes: 'All types',
      dateFrom: 'From',
      dateTo: 'To',
    },
    types: {
      players: {
        title: 'Players',
        description: 'Name, date of birth, category, status, jersey, position, and linked parents.',
      },
      users: {
        title: 'Users',
        description: 'Coaches, parents, and admins with email, role, status, and last login.',
      },
      categories: {
        title: 'Categories',
        description: 'Age range, assigned coach, player count, and status.',
      },
      matches: {
        title: 'Matches',
        description: 'Opponent, category, date, type, and status of scheduled matches.',
      },
    },
  },
  academySettings: {
    name: 'Academy name',
    logoUrl: 'Logo URL',
    logoPreviewOk: 'Logo preview',
    logoPreviewError: 'Could not load the image. Check the URL.',
    logoPreviewLoading: 'Loading preview…',
    contactEmail: 'Contact email',
    contactPhone: 'Phone',
    address: 'Address',
    timezone: 'Timezone',
    locale: 'Language',
    localeEs: 'Spanish (Panama)',
    localeEn: 'English (US)',
    currency: 'Currency',
    defaultPeriodsCount: 'Periods per match',
    defaultPeriodDuration: 'Period duration (min)',
    periodsHint:
      'These defaults apply when creating matches. You can override them per match later.',
    successUpdate: 'Settings saved',
    readOnlyBadge: 'Read only',
    currentPlan: 'Current plan',
    academyStatus: 'Academy status',
    billingAnchorDay: 'Billing anchor day',
    billingAnchorDayValue: 'Day {day} of each month',
    nextPeriodEnd: 'Next period end',
    notAvailable: 'Not available',
    billingStatus: 'Billing status',
    slug: 'Slug identifier',
    playersLimit: 'Active players',
    categoriesLimit: 'Categories',
    usageSummary: '{players} players · {categories} categories · {users} users',
    platformManagedHint: 'Plan, billing, and status are managed by the SquadVeloce platform.',
    contactProvider: 'To change plans or reactivate the academy, contact your service provider.',
    billingAlertTitle: 'Billing',
    billingAlertOverdue: 'You have an overdue invoice.',
    billingAlertPending: 'You have a pending invoice.',
    viewBilling: 'View billing',
    sections: {
      profile: 'Profile & logo',
      profileHint: 'Information visible to parents and coaches in your academy.',
      regional: 'Timezone, language & currency',
      gameConfig: 'Game periods',
      notifications: 'Parent notifications',
      platformManaged: 'Plan & billing',
    },
    notificationsEnabled: 'Send in-app notifications to parents',
    notificationsHint:
      'When disabled, no parent in the academy receives highlight alerts, even if their personal preferences are on.',
    validation: {
      nameRequired: 'Name is required',
      periodsInvalid: 'Enter between 1 and 6 periods',
      durationInvalid: 'Enter between 1 and 120 minutes',
    },
    status: {
      active: 'Active',
      suspended: 'Suspended',
      inactive: 'Inactive',
    },
    billingStatuses: {
      current: 'Current',
      pending: 'Payment pending',
      overdue: 'Overdue',
    },
  },
  theme: {
    toggle: 'Toggle theme',
    activateLight: 'Enable light theme',
    activateDark: 'Enable dark theme',
    light: 'Light theme',
    dark: 'Dark theme',
  },
  language: {
    toggle: 'Change language',
    es: 'Español',
    en: 'English',
    selectEs: 'Select Spanish',
    selectEn: 'Select English',
  },
  dataView: {
    viewCards: 'Cards',
    viewTable: 'Table',
    results: '{count} results',
    resultsOne: '1 result',
    noResults: 'No results match your search.',
    noResultsDescription: 'Try different terms or clear the filters.',
    sortAsc: 'Sort ascending',
    sortDesc: 'Sort descending',
    pagePrev: 'Previous',
    pageNext: 'Next',
  },
  a11y: {
    showPassword: 'Show password',
    hidePassword: 'Hide password',
    notifications: 'Notifications',
    openNav: 'Open navigation menu',
    closeNav: 'Close menu',
    collapseSidebar: 'Collapse sidebar',
    expandSidebar: 'Expand sidebar',
  },
  meta: {
    appDescription: 'Platform for youth football academies',
    loginTitle: 'Sign in — SquadVeloce',
  },
  onboarding: {
    checklist: {
      progress: '{done} of {total} completed',
      dismiss: 'Hide this list',
      allDone: "You've completed all the first steps!",
    },
    academyAdmin: {
      title: 'Getting started with your academy',
      category: {
        title: 'Create your first category',
        description: 'Set the age range and assign a coach.',
      },
      player: {
        title: 'Add a player',
        description: 'Register your first player and link them to their parents.',
      },
      parentLink: {
        title: 'Link a player to their parent or guardian',
        description: 'For underage players, connect their account with a parent or guardian.',
      },
      coach: {
        title: 'Invite a coach',
        description: 'Give a coach access so they can capture matches.',
      },
      match: {
        title: 'Schedule your first match',
        description: 'Set up a match with opponent, category, date, and location.',
      },
    },
  },
  tour: {
    dialogLabel: 'Screen guide',
    stepCount: 'Step {current} of {total}',
    back: 'Back',
    next: 'Next',
    finish: 'Got it',
    skip: 'Skip',
    replay: 'How does this screen work?',
    superAdmin: {
      home: {
        hero: { title: 'Your control center', body: 'See the whole platform at a glance: academies, users and revenue.' },
        kpis: { title: 'Key metrics', body: 'Active academies, total users, MRR and delinquency, all at a glance.' },
        billing: { title: "This month's billing", body: 'How much you billed, collected, and what\'s still pending this month.' },
        attention: { title: 'Needs attention', body: 'Academies suspended for billing and overdue invoices that need follow-up.' },
        quickLinks: { title: 'Quick links', body: 'Jump straight to plans, academies, billing or the audit log from here.' },
      },
      invoicesList: {
        kpis: { title: 'Invoices overview', body: 'Total billed, pending, overdue and paid for the selected month.' },
        monthFilter: { title: 'Change the month', body: 'Pick the billing period you want to review.' },
        processOverdue: { title: 'Process overdue', body: 'Flags unpaid invoices as overdue and auto-suspends academies when it applies.' },
        create: { title: 'Create an invoice', body: 'Create a manual invoice for an academy and a specific period.' },
      },
      auditLog: {
        kpis: { title: 'Total activity', body: "How many events were logged and what's the most frequent action." },
        filters: { title: 'Filter the log', body: 'Search by academy, user, entity, action or date range.' },
        viewDetail: { title: 'See the detail', body: 'Open an event to see exactly what changed and who did it.' },
      },
      plansList: {
        kpis: { title: 'Plans overview', body: 'How many plans you have in total, active and inactive.' },
        planDetails: { title: 'Price and limits', body: "Each plan shows its price and the limits the academy will see." },
        create: { title: 'Create a plan', body: 'Set the price, limits and status to offer it to academies.' },
        rowActions: { title: 'Edit or deactivate', body: "Update the plan's details or deactivate it so it's no longer available." },
      },
      planForm: {
        name: { title: 'Plan name', body: 'This is how academies will see it when choosing a plan.' },
        pricing: { title: 'Set the price', body: 'Annual fee and the per-player price billed every month.' },
        limits: { title: 'Plan limits', body: 'How many players, categories, users and matches it includes.' },
        submit: { title: 'Save the plan', body: "Create or update the plan so it's available right away." },
      },
      academiesList: {
        kpis: { title: 'Overall status', body: 'Total academies, active, suspended and platform users.' },
        create: { title: 'Register an academy', body: "Create the account, assign a plan and generate the admin's access." },
        rowActions: { title: 'Manage each academy', body: 'Approve requests, edit details, or suspend and reactivate access.' },
        billingStatus: { title: 'Billing status', body: 'See if the academy is current, overdue, or suspended for billing.' },
      },
      academyForm: {
        name: { title: 'Academy name', body: 'This is how it will be identified across the platform.' },
        plan: { title: 'Choose a plan', body: 'Sets the limits and the price that will be billed to it.' },
        billingAnchor: { title: 'Billing anchor day', body: "The day of the month its monthly invoice is generated." },
        submit: { title: 'Save changes', body: 'Creates the academy or updates its details, whichever applies.' },
      },
      academyDetail: {
        header: { title: 'Academy details', body: 'Plan, estimated billing, and its current and next billing periods.' },
        edit: { title: 'Edit the academy', body: 'Change the name, slug, plan or billing day.' },
        userKpis: { title: 'Academy users', body: 'Total users, plan limit, admins, coaches and parents.' },
        createUser: { title: 'Add a user', body: 'Create an admin, coach or parent right from here.' },
      },
      superAdmins: {
        kpis: { title: 'Active super admins', body: 'How many accounts have full access to the platform.' },
        createForm: { title: 'Invite a super admin', body: 'Grant full platform access with just their email.' },
        rowActions: { title: 'Activate or deactivate', body: "Remove a super admin's access without deleting their account." },
      },
      personalAccountsList: {
        kpis: { title: 'Account requests', body: 'Total, pending, approved and rejected.' },
        accountInfo: { title: 'Request details', body: "The account's name, request date and current status." },
        rowActions: { title: 'Approve or reject', body: 'Review the request and decide whether to grant the account access.' },
      },
    },
    academyAdmin: {
      home: {
        hero: { title: "Your academy at a glance", body: "Here you'll see your academy's name and top numbers: active players, categories, and upcoming matches." },
        kpis: { title: 'Key metrics', body: "A quick summary of players, categories, users, matches, and your billing plan." },
        attention: { title: 'What needs your attention', body: 'Important pending items will show up here, like players awaiting approval or invoices due.' },
        quickLinks: { title: 'Quick links', body: 'Jump straight to users, categories, players, matches, and billing from here.' },
      },
      billing: {
        planSummary: { title: 'Your current plan', body: "See your plan's name, price, and billing status here." },
        periodInfo: { title: 'Billing dates', body: 'Check your billing anchor day, current and next periods, and the due date.' },
        invoicesList: { title: 'Invoice history', body: 'Review your past invoices and download a PDF of any of them.' },
      },
      users: {
        kpis: { title: "Your academy's users", body: 'Total users and how many you have per role: admins, coaches, and parents.' },
        create: { title: 'Invite a user', body: 'Create accounts for coaches, parents, or other admins from here.' },
        list: { title: 'Search and filter users', body: 'Find users by name or email, filter by role or status, and manage each one.' },
      },
      categories: {
        kpis: { title: 'Your categories', body: 'How many categories you have in total, and how many still have no coach assigned.' },
        create: { title: 'Create a category', body: 'Set the age range, whether a guardian is required, and assign a coach.' },
        list: { title: 'Search and filter categories', body: 'Find categories by name, filter by status, and edit each one.' },
      },
      actions: {
        kpis: { title: 'Action catalog', body: 'How many actions are active, which ones notify parents, and their breakdown by impact.' },
        create: { title: 'Create an action', body: 'Set a code, name, impact, and whether it should notify parents when logged.' },
        list: { title: 'Search and filter actions', body: 'Find actions by name or code, and filter by impact or status.' },
      },
      players: {
        kpis: { title: 'Your players', body: 'Active players, those pending approval, and how many categories group them.' },
        create: { title: 'Add a player', body: 'Register a new player and link them to their parents or guardians.' },
        list: { title: 'Search and filter players', body: 'Find players by name, filter by status or category, and approve requests.' },
      },
      settings: {
        profile: { title: "Your academy's profile", body: "Update your academy's name, logo, and contact details." },
        regional: { title: 'Timezone and language', body: 'Adjust the timezone, language, and currency your academy uses.' },
        notifications: { title: 'Notifications', body: "Turn notifications to your academy's users on or off." },
        readonly: { title: 'Platform-managed data', body: 'Your plan, status, and usage limits are managed by the platform; contact us to change them.' },
      },
      reports: {
        hint: { title: "Your academy's reports", body: 'Generate exportable reports for players, users, categories, and matches.' },
        cardsGrid: { title: 'Filter before exporting', body: 'Each card has its own filters; adjust them before generating the report.' },
        exportPdf: { title: 'Export as PDF or CSV', body: 'Download the report in whichever format you prefer.' },
      },
    },
    coach: {
      home: {
        hero: { title: 'Your daily overview', body: "A quick glance at your categories and what's coming up." },
        kpis: { title: 'Key stats', body: 'Categories, upcoming matches, matches played this month, and live ones.' },
        attention: { title: 'What needs your attention', body: 'Live or upcoming matches that need action from you.' },
        quickLinks: { title: 'Quick links', body: 'Jump straight to categories, players, or matches from here.' },
      },
      categories: {
        grid: { title: 'Your categories', body: 'Each card summarizes a category you coach.' },
        card: { title: 'A look at each category', body: "Tap a card to see that category's full roster." },
        playerCount: { title: 'Called-up players', body: 'How many players the category has, and who they are, at a glance.' },
      },
      players: {
        categoryFilter: { title: 'Filter by category', body: 'Show only the players from the category you pick.' },
        grid: { title: 'Your roster', body: 'All the players called up across your categories.' },
        card: { title: 'Quick player card', body: "Each player's photo, jersey number, and category." },
        viewMatches: { title: 'View their matches', body: "Takes you to this player's category matches." },
      },
      analysis: {
        filters: { title: 'Filter the analysis', body: 'Combine category, match, dates, action type, and impact.' },
        summary: { title: 'Results summary', body: 'How many players, actions, and matches match the current filter.' },
        actions: { title: 'View and export', body: 'Switch between cards and table, or export the report as PDF or CSV.' },
        chart: { title: 'Player ranking', body: 'The players with the most actions under the applied filter.' },
        playerList: { title: 'Compare players', body: 'Tap a player to see their detailed analysis.' },
      },
      analysisPlayerDetail: {
        back: { title: 'Back to comparison', body: 'Returns to the player analysis list.' },
        header: { title: 'Player profile', body: 'Player info and the applied date or match filter.' },
        stats: { title: 'Performance summary', body: 'Matches, minutes, total actions, and their most frequent action.' },
        matches: { title: 'Match history', body: 'Every match played, with minutes and recorded actions.' },
        observations: { title: 'Coaching staff notes', body: 'Notes left by you or other coaches about this player.' },
      },
    },
    parent: {
      home: {
        hero: { title: 'Your main dashboard', body: "Here you'll see a quick summary for your children: matches, minutes, and stats." },
        childTabs: { title: 'Pick a child', body: 'If you have more than one registered, switch here to see their info.' },
        periodFilter: { title: 'Filter by period', body: 'Pick a month or view the full history to adjust the stats below.' },
        dashboardPanel: { title: 'Summary and stats', body: "Matches, minutes, and actions for your child will show up here as they're recorded." },
      },
      calendar: {
        overview: { title: 'Match calendar', body: "Check upcoming matches and review past ones; from here you can open each match's report card." },
      },
      notifications: {
        page: { title: 'Your notifications', body: "Control which alerts you get about your children's matches from here." },
        globalToggle: { title: 'Turn everything on or off', body: 'This switch controls whether you get in-app notifications. Turn it on or off anytime.' },
        emailNote: { title: 'More channels coming soon', body: 'For now notifications arrive in-app; email alerts are coming soon.' },
      },
      children: {
        page: { title: 'Your children', body: 'Manage your registered players: check their status and add a new one if needed.' },
        list: { title: 'Status and actions', body: 'Each card shows the approval status and lets you edit their info or view their match report cards.' },
      },
      childMatches: {
        back: { title: 'Back to your children', body: "Use this button to return to your children's list anytime." },
        list: { title: 'Matches and reports', body: 'Your child\'s matches show up here; open "View card" to check each one\'s detailed report.' },
      },
    },
    player: {
      home: {
        hero: { title: 'Your player summary', body: 'See your photo, category and jersey number here. This is how the academy recognizes you.' },
        goProfile: { title: 'Go to your profile', body: 'Tap here to check and update your personal details.' },
        goMatches: { title: 'Check your matches', body: 'Find your match history and report cards here.' },
        goReports: { title: 'Read your reports', body: 'See the notes your coaches leave about your progress.' },
      },
      profile: {
        avatar: { title: 'Change your photo', body: 'Tap your photo anytime you want to update it.' },
        info: { title: 'Jersey and category', body: "Your academy assigns these, so you can't edit them here." },
        editForm: { title: 'Edit your details', body: 'Update your name, birth date and position whenever you need to.' },
        save: { title: 'Save your changes', body: "Don't forget to tap here after editing your information." },
      },
      matches: {
        list: { title: 'Your match history', body: "Here you'll see every match your coach has recorded a report card for." },
        viewReport: { title: 'See your report card', body: 'Tap this button to review your actions in that match.' },
      },
      calendar: {
        root: { title: 'Your match calendar', body: "See your upcoming matches and the ones you've already played." },
        tabs: { title: 'Upcoming and recent', body: "Switch between your upcoming matches and the ones you've already played." },
        relativeLabel: { title: 'Countdown to your match', body: "We'll tell you how many days are left until your next match." },
      },
      reports: {
        intro: { title: 'Notes from your coaches', body: 'Read what they think about your progress in every match.' },
        observations: { title: 'Your observations history', body: "Find general comments and match-specific ones, all organized here." },
      },
    },
    shared: {
      matchesList: {
        kpis: { title: 'Match overview', body: 'See upcoming matches, matches in progress, and matches played this month at a glance.' },
        search: { title: 'Search for a match', body: 'Type the opponent, category, or location to find a match quickly.' },
        statusFilter: { title: 'Filter by status', body: 'Show only scheduled, in-progress, finished, or cancelled matches.' },
        categoryFilter: { title: 'Filter by category', body: 'Narrow the list to a single category when you manage several teams.' },
        createButton: { title: 'Create a match', body: 'Schedule a new match with opponent, category, date, and location.' },
      },
      matchDetail: {
        header: { title: 'Match details', body: 'See the opponent, category, match type, and current status here.' },
        summary: { title: 'General summary', body: 'Check the date, location, playing periods, and who created the match from the Overview tab.' },
        clock: { title: 'Match clock', body: 'Control the current minute and period: pause, resume, or move to the next period.' },
        players: { title: 'Pick the player', body: 'Tap a player on the pitch to start recording a play for them.' },
        actions: { title: 'Record the play', body: 'With the player selected, tap the action (goal, foul, assist...) to record it instantly.' },
        lastPlay: { title: 'Last recorded play', body: 'Confirm what you just logged here. You have a few seconds to undo it if you make a mistake.' },
        voice: { title: 'Voice capture', body: 'Turn on the mic and say the jersey number and action ("5, goal") to record plays hands-free.' },
      },
      matchReportCard: {
        header: { title: 'Player card', body: 'Photo, jersey number, and match details: opponent, category, and date.' },
        chart: { title: 'Match performance', body: "Switch between radar and bars above to see the player's actions by dimension." },
        motivation: { title: 'Featured message', body: "A motivational phrase based on the player's strongest point in this match." },
        footer: { title: 'Minutes and sharing', body: 'Check minutes played and share the card with a link.' },
        observations: { title: "Coach's observations", body: "The coach's notes about this player appear here, both general and match-specific." },
      },
    },
  },
} as const;
