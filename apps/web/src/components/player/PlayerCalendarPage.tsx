...
function MatchCalendarCard({
  ...
  const relativeLabel = useMemo(() => {
    if (item.status === MatchStatus.IN_PROGRESS) {
      return t('dashboard.player.calendar.inProgress');
    }
    if (variant !== 'upcoming') return null;
    const days = daysUntilMatch(item.matchDatetime, timezone);
    if (days <= 0) return t('dashboard.player.calendar.today');
    if (days === 1) return t('dashboard.player.calendar.tomorrow');
    return t('dashboard.player.calendar.inDays', { count: days });
  }, [item.matchDatetime, item.status, timezone, t, variant]);

  const reportPath = appPath(`/dashboard/player/matches/${item.matchId}`);
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <h4 className="font-display text-xl font-semibold tracking-tight text-text-primary sm:text-2xl">
              {t('dashboard.player.calendar.vsOpponent', { opponent: item.opponent })}
            </h4>
          ...
          {item.location && (
            <p className="flex items-start gap-1.5 text-sm text-text-muted">
              ...
              <span>{t('dashboard.player.calendar.location', { location: item.location })}</span>
            </p>
          )}
        </div>
        ...
        {variant === 'past' && (
          ...
          >
            {t('dashboard.player.calendar.viewReportCard')}
          </Button>
        )}
      </div>
    </article>
    ...
    <div className="ds-stagger-enter space-y-5">
      {isEmpty ? (
        <EmptyState
          ...
          description={t('dashboard.player.calendar.emptyDescription')}
        ...
        <>
          ...
          >
            {(
              [
                ['upcoming', t('dashboard.player.calendar.upcomingSection'), upcoming.length],
                ['past', t('dashboard.player.calendar.pastSection'), past.length],
              ] as const
            ).map(([id, label, count]) => {
              const active = tab === id;
              return (
          ...
          {tab === 'upcoming' && upcoming.length === 0 ? (
            <p className="text-sm text-text-secondary">
              {t('dashboard.player.calendar.noUpcoming')}
            </p>
          ) : (
            <ul className="space-y-3">
              {visible.map((item, index) => (
                <li
                  key={`${item.matchId}-${item.playerId}`}
